...
$(document).ready () ->
	...
	### --- On Ca --- ###
	$(document).on "click", "#category-select #level-one-category input[name='categories']", () ->
		$("#category-select #level-two-category").addClass "hidden"
		$("#category-select #level-one-category").removeClass "hidden"
		# parent_category = $(this).parent().find("div.interested-label").text().replace(/\n/g, '').replace(/  /g, '')
		# $("#category-select #level-two-category span#main-cat-name").text(parent_category)
		# $("#category-select #level-two-category h5#main-cat-title").text(parent_category)
		# $("#category-select #level-two-category").removeClass "hidden"
		# $("#category-select #level-one-category").addClass "hidden"
		return
