...
trait Taggable
{
	 ...
	 * @return array
	 */
	public function tagNames($groups)
	{

		$all_tags_in_group = $this->existingTagsInGroups([$groups])->pluck('name')->toArray();
		$all_listing_tags = $this->tagged->map(function($item){
			return $item->tag_name;
		})->toArray();
		return array_intersect($all_tags_in_group,$all_listing_tags);
	}
	 ...
	 */
	public function untag($group,$tagNames=null)
	{
		if(is_null($tagNames)) {
			$tagNames = $this->tagNames($group);
		}
	 ...
	 * @param $tagName string or array
	 */
	public function retag($group_name,$tagNames)
	{
		
		$tagNames = static::$taggingUtility->makeTagArray($tagNames);
		$currentTagNames = $this->tagNames($group_name);
		
		$deletions = array_diff($currentTagNames, $tagNames);
		$additions = array_diff($tagNames, $currentTagNames);
		
		$this->untag($group_name,$deletions);

		foreach($additions as $tagName) {
			$this->addTag($group_name,$tagName);
		}
	}
	 ...
	 */
	private function addTag($group,$tagName)
	{
