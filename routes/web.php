...
Route::group([], function() {
	/* Autopopup setup - Not set on Helper as the css & js urls are also read by the url()->current() */
	/*$defaults_routes = Defaults::where([['label', 'ban_autopop_up_route_path'], ['type', 'enquiry_flow']])->get()->first();
	
	if(!$defaults_routes) // If null
		$defaults_routes = Defaults::create(['label' => 'ban_autopop_up_route_path', 'meta_data' => serialize(['/news']), 'type' => 'enquiry_flow']);

	$cookie_class_obj = new CookieController;
	$route_split = url()->current();
	$route_split = str_replace(config('app.url'), "", $route_split);
	
	if(in_array($route_split, unserialize($defaults_routes->meta_data))) { // Make the value -ve if the route comes under the list of Banned URLs
		$cookie_val = cookie()->get('enquiry_modal_display_count');
		$cookie_class_obj->set('enquiry_modal_display_count', 0 - intVal($cookie_val), ['http_only' => false]); // Set the Auto Enquiry Modal Popup count to ZERO, as the User did an Enquiry
	} else {// Make the value +ve if the route doesn't comes under the list of Banned URLs
		$cookie_val = cookie()->get('enquiry_modal_display_count');
		$cookie_class_obj->set('enquiry_modal_display_count', abs($cookie_val), ['http_only' => false]);
	}*/
